...
class EasyApplyBot:
    ...
    #MAX_APPLICATIONS = int(num_label)

    def __init__(self,username,password, premium, phone, language, num_label, position, location, resumeloctn, appliedJobIDs, filename):

        print("\nWelcome to Easy Apply PanBot 2.0\n")
        dirpath = os.getcwd()
        ...
        self.autofill(phone, resumeloctn)
        ...
        while count_application < self.MAX_APPLICATIONS:
            ...
            for jobID in jobIDs:
                ...
                if button is not False:
                    ...
                    self.send_inmail()
                    count_application += 1
                    print("Panbot applied for " + count_application + "out of " + self.max_applications)
                else:
                ...
                if self.browser.find_element_by_css_selector('button[aria-label = "Continue to next step"]').is_displayed():
                    ...
                    while running2:
                        try:
                            ...
                            next_button.click()
                            if self.browser.find_element_by_xpath("//p[contains(.,'Invalid answer')]").is_displayed():
                                close_button = self.wait.until(EC.element_to_be_clickable((By.CSS_SELECTOR, "button[aria-label='Dismiss']")))
                                close_button.click()
                                discard_button = self.browser.find_element_by_xpath("//button[contains(.,'Discard')]")
                                discard_button.click()
                                print("Some answers could not be automated, cant apply for job")
                                running2 = False
                                running1 = False
                            else:
                                continue
    ...
    def autofill(self, phone, resumeloctn):
        self.browser.get("https://www.linkedin.com/jobs/application-settings/")
        time.sleep(5)

        if self.browser.find_element_by_id("application-settings-onsite").is_enabled():
            while True:
                try:
                    self.browser.find_element_by_xpath("//span[contains(.,'Delete')]").click()
                    time.sleep(2)
                except NoSuchElementException:
                    time.sleep(2)
                    self.browser.find_element_by_id("phone-information").send_keys(phone)
                    self.browser.execute_script("window.scrollTo(0,500);")
                    self.browser.find_element_by_id("jobs-resume-manager__file-upload").send_keys(resumeloctn)
                    time.sleep(3)
                    if self.browser.find_element_by_id("application-settings-external").is_enabled():
                        break
                    else:
                        self.browser.find_element_by_id("application-settings-external").click()
                    break
        else:
            self.browser.find_element_by_id("application-settings-onsite").click()
            while True:
                try:
                    self.browser.find_element_by_xpath("//span[contains(.,'Delete')]").click()
                    time.sleep(2)
    ...
    def finish_apply(self):
        print("Panbot successfuly completed your job search!")
        print("Please refer to complete_joblist.csv for more information")
        self.browser.close()
...
if __name__ == '__main__':
    ...
    if useGUI == True:
        ...
        phone = app.frames["StartPage"].phone
    ...
    # start bot
    bot = EasyApplyBot(username, password, premium, phone, num_label, language, position, location, resumeloctn, appliedJobIDs, filename)
